...
test('adding a company creates a persistent star and flies to it', async ({ page }) => {
  ...
  const pendingKinds = await page.evaluate(() => {
    const raw = sessionStorage.getItem('enterprise-ai-galaxy:pending-toasts');
  ...
  const persisted = await page.evaluate(() => {
    const raw = localStorage.getItem('enterprise-ai-galaxy:user-stars');
  ...
  await page.evaluate(() => {
    ...
    if (raw) {
      ...
      sessionStorage.setItem(
        'enterprise-ai-galaxy:pending-toasts',
...
test('a hydrated toast keeps only its remaining window (no fresh duration)', async ({ page }) => {
  ...
  await page.evaluate(
    ({ star, ms }) => {
      localStorage.setItem('enterprise-ai-galaxy:user-stars', JSON.stringify([star]));
      sessionStorage.setItem(
        'enterprise-ai-galaxy:pending-toasts',
        JSON.stringify([
